"""
main.py — Точка входа VoiceTerm. Выбирает режим: настройка или рабочий.
"""
...
def run_setup_mode():
    """Запустить режим первоначальной настройки (WiFi AP + веб-интерфейс)."""
    ...
    if os.path.exists(ap_script):
        ...
        logger.warning("setup_ap.sh не найден, AP не поднята")
...
class VoiceTermApp:
    """Основное приложение рабочего режима."""
    ...
    def _on_wakeword(self, confidence: float):
        ...
        if not self._recording_lock.acquire(blocking=False):
            logger.info("Запись уже идёт, пропускаю")
            return
        try:
    ...
    def run(self):
        ...
        logger.info("Система запущена. Ожидание ключевого слова...")
        try:
            while True:
